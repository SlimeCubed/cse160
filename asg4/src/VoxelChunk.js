...
void main() {
    ...
    v_Color = a_Color;
    v_Uv = a_Uv;
    v_Submersion = -0.5 - worldPosition.y;
...
void main() {
    ...
    gl_FragColor.a = 1.0;
    gl_FragColor.rgb = light(v_WorldPosition, v_Normal, color.rgb, v_ViewDir);
    gl_FragColor.rgb = mix(gl_FragColor.rgb, vec3(0.1, 0.15, 0.2) * (u_DoLighting == 1 ? u_AmbientColor / vec3(0.6, 0.6, 0.8) : vec3(1.0, 1.0, 1.0)), clamp(v_Submersion / 9.0 + float(v_Submersion > 0.0) * 0.5, 0.0, 1.0));
}
`;
